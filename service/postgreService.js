...
export const createCompanyData = async (query = '') => {
    try {
        ...
        console.error('Error creating data on DB:', error);
        // Return error message on query failure  
        return JSON.stringify({ message: PostGreSqlDbMessages.INVALID_SQL_MESSAGE });
    }
}
...
export const getCompanyData = async (query = '') => {
    try {
        ...
        if (query != '') {
            ...
            console.log('db response: ',modifiedResponse);
            return JSON.stringify(modifiedResponse);
        }
        else {
            // Return alert message if the query is not appropriate 
            return JSON.stringify({ message: PostGreSqlDbMessages.TRY_AGAIN_MESSAGE });
        }        
    } catch (error) {
        console.error('Error fetching data:', error);
        // throw error;
        // Return Error message on failure 
        const errorMessage = error.message || error.toString(); // Safely handle both cases
        return errorMessage;
    }
