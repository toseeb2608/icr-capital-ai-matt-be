...
import { generateGeminiResponse, getGeminiAIPromptService } from '../service/geminiAiPromptService.js';
...
import { generateClaudeResponse, } from '../service/claudeAiPromptService.js';
...
export const fetchprompts = async (req, res, next) => {
	...
	try {
		console.log('fetchprompts called for threadid:', threadid);
		...
		console.log(`Found ${prompts.length} prompts for thread ${threadid}`);
		...
		console.log(`After deduplication: ${uniquePrompts.length} unique prompts`);
...
export const fetchChatThreads = async (req, res, next) => {
	...
	try {
		...
		let assistantThreads = await getAssistantThreadsByQuery({ user: new mongoose.Types.ObjectId(userid), is_deleted: false });
 ...
 * @param {Object} req - Request object, expected to contain 'userid', 'threadId', 'lastPrompt', and 'promptId' in the body.
 * @param {Object} res - Response object.
 * @param {Function} next - Next function for error handling.
...
export const updateEditedPrompt = async (req, res, next) => {
	const { userid, threadId, lastPrompt, promptId } = req.body;
	try {
		console.log('updateEditedPrompt called with:', { userid, threadId, lastPrompt, promptId });
		
		// Find and update the specific prompt by ID
		const result = await promptModel.findByIdAndUpdate(
			promptId,
			{ 
		...
		console.log('updateEditedPrompt result:', result);
		...
		console.error('updateEditedPrompt error:', error);
		next(InternalServer(PromptMessages.INTERNAL_SERVER_ERROR));
	}
};
...
export const regeneratePrompt = async (req, res, next) => {
	...
	try {
		console.log('regeneratePrompt called with:', { id, userPrompt });
		console.log('ID type:', typeof id, 'ID value:', id);
		
		// Validate ObjectId format
		if (!mongoose.Types.ObjectId.isValid(id)) {
			console.log('Invalid ObjectId format:', id);
			return next(BadRequest('Invalid prompt ID format'));
		}
		
		const existingPrompt = await promptModel.findById(id);

		if (!existingPrompt) {
			console.log('Prompt not found with ID:', id);
			return next(BadRequest(PromptMessages.PROMPT_NOT_FOUND));
		}

		console.log('existingPrompt found:', {
			_id: existingPrompt._id,
			description: existingPrompt.description,
			threadid: existingPrompt.threadid
		});
		...
		switch (botProvider) {
			case 'openai':
				botResponse = await generateOpenAIResponse({ userPrompt });
			...
			case 'claude':
				botResponse = await generateClaudeResponse({prompt: userPrompt})
		...
		console.log('botPayload to be saved:', botPayload);
		
		// Update the prompt directly using findByIdAndUpdate
		...
		console.log('Direct update result:', updatedPrompt);
		
		if (!updatedPrompt) {
			console.log('Failed to update prompt with ID:', id);
			return next(BadRequest(PromptMessages.PROMPT_NOT_FOUND));
		}
		
		console.log('regeneratePrompt completed successfully');
		res.status(StatusCodes.OK).json({
			success: true,
			message: PromptMessages.UPDATED_SUCCESSFULLY,
			data: botResponse,
