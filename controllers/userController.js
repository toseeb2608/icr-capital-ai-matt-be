...
export const getAllUsers = async (req, res) => {
	...
	try {
		...
		if(role=='ceda')
		{
			filterOptions = {
				...regex,
				// role: { $ne: UserRole.SUPER_ADMIN, $ne: UserRole.CEDA },
				role: { $nin: [UserRole.SUPER_ADMIN, UserRole.CEDA] },
		...
		if (role !== UserRole.SUPER_ADMIN|| role!==UserRole.CEDA) {
...
export const UpdateUser = async (req, res, next) => {
	const { id } = req.params;
	const {
		body: { fname, lname, email, teams, status, role },
	} = req;
	...
	try {
		...
		const response = await axios.post(`${process.env.REGISTER_DJANGO_API_URL}update-user`, {
			fname,
			email,
		}, {
			headers: {
		...
		// If status is changed to active, update related notifications
		if (status === 'active') {
...
export const softUserDelete = async (req, res, next) => {
    ...
    const email = user.email;
		...
		const response = await axios.post(`${process.env.REGISTER_DJANGO_API_URL}delete-user`, {
			username,
			email,
		}, {
			headers: {
...
export const getNotificationCounts = async (req, res) => {
  try {
    // Count notifications for inactive users
    const inactiveUsersCount = await Notification.countDocuments({
      "details.status": "pending",
      isRead: false 
    });
